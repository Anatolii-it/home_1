...
int pdoba; // потужність приладів яки працюють цілодобово
...
void searchEmployee() {
    ...
    for (const auto& emp : employees) {
        if (emp.phone == phone) {
            ...
            cin >> h;
            p = double(emp.kWt_month - (pdoba * 31)) / 31 / s;
            cout << "потужність -  " << p << endl;
            a = double((emp.kWt_month / 31 * emp.time_day) / 24);
            if (a >= 0.0 && a <= 2) {
